...
import com.forrestguice.suntimeswidget.map.WorldMapMollweide;
...
public class SunPosLayout_3X2_0 extends SunPosLayout
{
    ...
    public static WorldMapProjection createProjectionForMode(Context context, WorldMapWidgetSettings.WorldMapWidgetMode mapMode, WorldMapOptions options)
    {
        ...
        switch (mapMode)
        {
            ...
            case MOLLWEIDE_SIMPLE:
                options.map = (background != null) ? background : ContextCompat.getDrawable(context, R.drawable.worldmap_mollweide);
                options.map_night = null;
                options.hasTransparentBaseMap = true;
                projection = new WorldMapMollweide();
                break;
