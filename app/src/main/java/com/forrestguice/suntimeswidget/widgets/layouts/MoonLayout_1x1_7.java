...
public class MoonLayout_1x1_7 extends MoonLayout
{
    ...
    public void updateViews(Context context, int appWidgetId, RemoteViews views, SuntimesMoonData data)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN)
        {
            if (WidgetSettings.loadScaleTextPref(context, appWidgetId))
            {
                ...
                float[] adjustedSizeSp = adjustTextSize(context, maxDp, paddingDp, "sans-serif", boldTime, "000,000.0 MM", timeSizeSp, SuntimesLayout.MAX_SP, "", suffixSizeSp);
    ...
    {
        TimeDisplayText distanceDisplay = LengthUnitDisplay.formatAsDistance(AndroidResources.wrap(context), moonPosition.distance, units, PositionLayout.DECIMAL_PLACES, true);
