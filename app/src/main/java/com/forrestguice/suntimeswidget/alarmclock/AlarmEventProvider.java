        ...
        for (int i=0; i<columns.length; i++)
        {
            switch (columns[i])
            {
                case COLUMN_EVENT_TIMEMILLIS:
                    ...
                    Calendar calendar = AlarmScheduler.updateAlarmTime_solarEvent(AndroidSuntimesDataSettings.wrap(context), event, location, offset, repeating, repeatingDays, now);
        ...
        for (int i=0; i<columns.length; i++)
        {
            switch (columns[i])
            {
                case COLUMN_EVENT_TIMEMILLIS:
                    ...
                    Calendar calendar = ShadowRatioEvent.updateAlarmTime(context, event, getClockData(context, location), offset, repeating, repeatingDays, now);
                    if (calendar != null) {
                        row[i] = calendar.getTimeInMillis();
                    }
                    break;
