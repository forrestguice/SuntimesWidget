 ...
 * AlarmClockActivity
 */
public class AlarmClockActivity1 extends AppCompatActivity
{
    ...
    protected AlarmClockItem createAlarm(AlarmClockItem.AlarmType type, String label, SolarEvents event, int hour, int minute, boolean vibrate, Uri ringtoneUri, ArrayList<Integer> repetitionDays, boolean addToDatabase)
    {
        ...
        alarm.enabled = AlarmSettings.loadPrefAlarmAutoEnable(AlarmClockActivity1.this);
        alarm.type = type;
        alarm.label = label;
        ...
        alarm.minute = minute;
        alarm.event = event;
        alarm.location = WidgetSettings.loadLocationPref(AlarmClockActivity1.this, 0);
        ...
        {
            Ringtone ringtone = RingtoneManager.getRingtone(AlarmClockActivity1.this, ringtoneUri);
            alarm.ringtoneName = ringtone.getTitle(AlarmClockActivity1.this);
            ringtone.stop();
        }
        ...
        if (showWarnings && notificationWarning.shouldShow() && !notificationWarning.wasDismissed())
        {
            ...
            notificationWarning.getSnackbar().setAction(getString(R.string.configLabel_alarms_notifications), new View.OnClickListener()
            {
                ...
                public void onClick(View view) {
                    SuntimesSettingsActivity.openNotificationSettings(AlarmClockActivity1.this);
                }
            });
    ...
    {
        boolean recreateActivity = ((!AppSettings.loadThemePref(AlarmClockActivity1.this).equals(appTheme))                           // theme mode changed
                //       || (appThemeOverride != null && !appThemeOverride.themeName().equals(getThemeOverride()))                       // or theme override changed
                || (localeInfo.localeMode != AppSettings.loadLocaleModePref(AlarmClockActivity1.this))                             // or localeMode changed
                || ((localeInfo.localeMode == AppSettings.LocaleMode.CUSTOM_LOCALE                                              // or customLocale changed
                && !AppSettings.loadLocalePref(AlarmClockActivity1.this).equals(localeInfo.customLocale)))
        );
        if (recreateActivity) {
                ...
                if (ActivityCompat.shouldShowRequestPermissionRationale(this, android.Manifest.permission.READ_EXTERNAL_STORAGE))
                {
                    ...
                    requestDialog.setPositiveButton(getString(android.R.string.ok), new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if (Build.VERSION.SDK_INT >= 16) {
                                ActivityCompat.requestPermissions(AlarmClockActivity1.this, new String[] { android.Manifest.permission.READ_EXTERNAL_STORAGE }, REQUEST_STORAGE_PERMISSION );
                            }
                        }
                    });
